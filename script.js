...
if (footerCopy) footerCopy.innerHTML = 'IT services &amp; digital solutions company.<br>Based in Nepal. Working worldwide.';
// Keep this copy static so CMS responses cannot replace it with stale data.
...
const emailChoices = [
  ['info@madebycodecraft.com', 'mailto:info@madebycodecraft.com'],
...
if (contactEmailLink) {
  ...
  contactEmails.className = 'contact-direct-emails';
  emailChoices.forEach(([label, href]) => {
    const link = document.createElement('a');
    link.href = href;
    link.textContent = label;
    contactEmails.append(link);
  });
  contactEmailLink.replaceWith(contactEmails);
}
