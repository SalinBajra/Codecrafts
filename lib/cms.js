...
const MIN_PUBLIC_CONTENT_VERSION = 2;
...
async function readContent() {
  ...
  // Supabase REST can sit behind an intermediary cache. The CMS is an
  // authoring surface, so a public read must always observe the latest row.
  const endpoint = `${supabaseUrl()}/rest/v1/site_content?key=eq.primary&select=payload&limit=1`;
  const response = await fetch(endpoint, {
    cache: 'no-store',
    headers: { ...supabaseHeaders(), 'cache-control': 'no-cache, no-store', pragma: 'no-cache' }
  });
  ...
  if (Number(rows[0].payload.version || 0) < MIN_PUBLIC_CONTENT_VERSION) {
    return { content: fallbackContent(), source: 'supabase-stale' };
  }
...
async function writeContent(content) {
  ...
  const response = await fetch(endpoint, {
    method: 'POST',
    cache: 'no-store',
    headers: { ...supabaseHeaders(), Prefer: 'resolution=merge-duplicates,return=minimal', 'cache-control': 'no-cache, no-store', pragma: 'no-cache' },
